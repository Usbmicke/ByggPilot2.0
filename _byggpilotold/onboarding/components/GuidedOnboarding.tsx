...
import { useUser } from '@/app/_hooks/useUser'; // Importera useUser-hooken
...
const GuidedOnboarding: FC = () => {
  ...
  const { user } = useUser(); // Hämta användaren från hooken
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('/api/genkit/flows/generateUploadUrl', {
        method: 'POST',
        headers,
  ...
  const handleSaveCompanyInfo = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      try {
        const headers = await getAuthHeader();
        const response = await fetch('/api/genkit/flows/saveCompanyInfo', {
          method: 'POST',
          headers,
  ...
  const handleCreateDriveFolders = async () => {
    startTransition(async () => {
      ...
      try {
        const headers = await getAuthHeader();
        const response = await fetch('/api/genkit/flows/createDriveFolders', {
          method: 'POST',
          headers,
        });
